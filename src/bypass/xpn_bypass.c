...
int open ( const char *path, int flags, ... )
{
  ...
  debug_info("[BYPASS] << After open....%d\n", ret);
...
int open64 ( const char *path, int flags, ... )
{
  ...
  debug_info("[BYPASS] << After open64....%d\n", ret);
...
int newfstatat ( __attribute__((__unused__)) int ver, int dirfd, const char *path, struct stat *buf, int flags )
{
  int  ret = -1;

  debug_info("[BYPASS] >> Begin __NR_newfstatat...\n");
  debug_info("[BYPASS]    * ver:   %d\n", ver);
  debug_info("[BYPASS]    * path:  %s\n", path);
  debug_info("[BYPASS]    * dirfd: %d\n", dirfd);
  debug_info("[BYPASS]    * flags: %o\n", flags);
  ...
  {
    debug_info("[BYPASS]\t dlsym_newfstatat %d,%p,%p,%d\n", dirfd, path, buf, flags);

    ret = dlsym_fstatat(dirfd, path, buf, flags);

    debug_info("[BYPASS]\t dlsym_newfstatat %d,%p,%p,%d -> %d\n", dirfd, path, buf, flags, ret);
  }

  debug_info("[BYPASS] << After __NR_newfstatat...\n");

  return ret;
}

int newfstatat64 ( __attribute__((__unused__)) int ver, int dirfd, const char *path, struct stat64 *buf, int flags )
{
  int    ret = -1;
  struct stat st;

  debug_info("[BYPASS] >> Begin __NR_newfstatat64...\n");
  debug_info("[BYPASS]    * ver:   %d\n", ver);
  debug_info("[BYPASS]    * dirfd: %d\n", dirfd);
  debug_info("[BYPASS]    * path:  %s\n", path);
  debug_info("[BYPASS]    * buf:   %p\n", buf);
  ...
  debug_info("[BYPASS] << After __NR_newfstatat64...\n");
