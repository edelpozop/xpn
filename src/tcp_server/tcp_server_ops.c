...
void on_subscribe(struct mosquitto * mosqtcpserver, void * obj, int mid, int qos_count,
...
void on_message(struct mosquitto * mosqtcpserver, void * obj,
    const struct mosquitto_message * msg) {
    int rc, headersize = 100, bytes, pos, block, flags;
    char payloadh[headersize], topic[TCP_SERVER_MAXPATHLEN];
    char * op, * rest;
    printf("AAAAAAAAAAAAAAAAA\n");
    bzero(payloadh, headersize);
    bzero(topic, TCP_SERVER_MAXPATHLEN);

    sprintf(topic, "%s", msg -> topic);
    printf("[%d]\t%ld\n", __LINE__, strlen(msg -> payload));
    /*sprintf(payload, "%s", (char *) msg->payload);
    //printf("payload %s\n", payload);
    op = strtok (payload, "|");
    ...
    else if (strcmp(op, "write") == 0)
    {
      ...
      int fd = open("prueba.txt", O_WRONLY | O_CREAT, 0777);
      int ret = lseek(fd,pos,SEEK_SET);
      int sz = write(fd, buf, bytes);
      close(fd);
...
void tcp_server_op_open(int sd, struct st_tcp_server_msg * head) {
    ...
    if (mosqtcpserver == NULL) init_mosquitto();
...
void tcp_server_op_creat(int sd, struct st_tcp_server_msg * head) {
    ...
    rc = mosquitto_subscribe(mosqtcpserver, NULL, head -> u_st_tcp_server_msg.op_creat.path, 0);
    printf("[%ld] %d\n\n", __LINE__, rc);
    if (rc != MOSQ_ERR_SUCCESS) {
        fprintf(stderr, "Error subscribing open: %s\n", mosquitto_strerror(rc));
        mosquitto_disconnect(mosqtcpserver);
...
void tcp_server_op_write(int sd, struct st_tcp_server_msg * head) {
    ...
    /*printf("[%d]\tBEGIN OPEN (write) MOSQUITTO TCP_SERVER - %s\n\n", __LINE__, head -> u_st_tcp_server_msg.op_write.path);
    ...
    printf("[%d]\tEND OPEN (write) MOSQUITTO TCP_SERVER - %s\n\n", __LINE__, head -> u_st_tcp_server_msg.op_write.path);*/
