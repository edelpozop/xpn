...
int nfi_tcp_server_init(char * url, struct nfi_server * serv, struct nfi_attr_server * attr) {
  ...
  rc = mosquitto_connect(mosqstr, "localhost", 1883, 60);
  ...
  printf("[%s|%d] MOSQUITTO LOOP START %d\n\n", __FILE__,__LINE__, rc);
  ...
  ssize_t nfi_tcp_server_write(struct nfi_server * serv,
    ...
    //off_t offset)
    {
      ...
      do {
        ...
        rc = mosquitto_publish(mosqstr, NULL, topic, 100, payload, 0, false);
        //
        // bzero(topic, 255);
        // bzero(payload, MAX_BUFFER_SIZE + 100);
        //
        if(rc != MOSQ_ERR_SUCCESS){
        ...
        rc = mosquitto_publish(mosqstr, NULL, topic, bytes_to_send, (char * ) buffer + cont, 0, false);
        //
        // bzero(topic, 255);
        // bzero(payload, MAX_BUFFER_SIZE + 100);
        //
      ...
      // {
      // 	int bytes_to_send = 0;
      // 	if (remaining_bytes > buffer_size) bytes_to_send = buffer_size;
      // 	else bytes_to_send = remaining_bytes;
      //
      // 	strncpy(buff, buffer+pos, bytes_to_send);
      // 	sprintf(payload,"write|%d|%d|%d|", bytes_to_send, pos, i);
      // 	strcat(payload, buff);
      //
      // 	rc = mosquitto_publish(mosqstr, NULL, topic, strlen(payload), payload, 0, false);
      // 	//printf("%d bytes sent; %ld remaining_bytes; %d pos\n", bytes_to_send, remaining_bytes, pos);
      //
      // 	if(rc != MOSQ_ERR_SUCCESS){
      // 		fprintf(stderr, "Error publishing write: %s\n", mosquitto_strerror(rc));
      // 		return -1;
