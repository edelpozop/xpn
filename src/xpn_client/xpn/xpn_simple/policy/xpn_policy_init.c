    ...
    case CONF_MXML:
        ...
        if (node != NULL) {
            for (node = mxmlFindElement(node, fconf -> connect_u.xml.conf_partition_node,
                    XML_TAG_ELEM_DATANODE, NULL, NULL, MXML_DESCEND); node != NULL; node = mxmlFindElement(node, fconf -> connect_u.xml.conf_partition_node,
        ...
        if (part -> data_nserv > 1) {
            ...
            if (node != NULL) {
                for (node = mxmlFindElement(node,
                        ...
                        XML_TAG_ELEM_METADATANODE, NULL, NULL, MXML_DESCEND); node != NULL; node = mxmlFindElement(node, fconf -> connect_u.xml.conf_partition_node,
